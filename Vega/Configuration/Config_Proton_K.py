...
######################################### Setup ########################################
    ...
    mission.payload=18000                 #Mission Payload to carry
    ...
    rocket.mass=689000                  #Initial Rocket mass (comparison only)
    ...
    Diameter=[7.4,4.1,4.1]      #Stage diameter (mandatory)
    length=[20.2,17.1,4.1]     #Stage length (*)
...
#    Thrust=[2261000,1196000,260000,2420]*rocket.num_stages      #Stage Thrust (depends on the user choice)
    Thrust=[10450000,237000*g0,62000*g0]      #Stage Thrust (depends on the user choice)


    TWR=[2]*rocket.num_stages                       #Thrust to Weight ratio (depends on the user choice)
    
    Isp=[316,327,327]                   #Specific impulse of each stage (mandatory)

    Num_engines=[6,4,1]             #Number of engines of each stage (mandatory)
    stage_type=["liquid","liquid","liquid"] #Type of propellant (mandatory)

    OF_ratio=[2.67,2.67,2.67]       #Propellant Oxidizer Fuel Ratio. (Only in the presence of liquid stages)
                                    #The array needs to be of the same size as the number of stages included

    Oxidizer_density=[1450,1450,1450]       #The oxidizer density for liquid stages only
    Fuel_density=[793,793,793]            #The fuel density for liquid stages only
 ...
 #   propellant_mass=[88365,23906,10115,550]*rocket.num_stages                        #Propellant Mass of the stage (comparison only)
    ...
    total_mass=[450410,167863,50747]           #Total Mass of the stage (comparison only)
    propellant_mass=[419410,156113,46562]                        #Propellant Mass of the stage (comparison only)
